...
public class CancelController {
    ...
    public HttpEntity calculateRefund(@PathVariable String orderId, @RequestHeader HttpHeaders headers) {
        LOGGER.info("[calculateRefund][Calculate refund amount][orderId: {}]", orderId);
        return ok(cancelService.calculateRefund(orderId, headers));
    }

    @CrossOrigin(origins = "*")
    @GetMapping(path = "/cancel/{orderId}/{loginId}")
    public HttpEntity cancelTicket(@PathVariable String orderId, @PathVariable String loginId, @RequestHeader HttpHeaders headers) {
        LOGGER.info("[cancelTicket][Cancel ticket][orderId: {}, loginId: {}]", orderId, loginId);
        return ok(cancelService.cancelTicket(orderId, loginId, headers));
    }
}